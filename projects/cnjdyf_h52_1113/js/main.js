...
//music1
$('#music1 .dialog .page-back').on('click',function(){$('#music1 .dialog').addClass('hide');m1.currentTime = 0;initAudio('m1');});
...
//music2
$('#music2 .dialog .page-back').on('click',function(){$('#music2 .dialog').addClass('hide');m2.currentTime = 0;initAudio('m2');});
...
var initAudio = function(id){
	...
	$('#'+id).siblings('.page-control').find('.time-end').html(totalTime);
...
function updateProgressVal(){
	var progressVal=parseInt(audio.currentTime/audio.duration*100)+'%';
	// document.getElementById("musicAudioProgress").style.width=progressVal+"px";
	$('.progress-val').width(progressVal);
	// $('#'+id).siblings('.page-control').find('.progress-val').css('width',progressVal)
	console.log('进度条当前值'+progressVal);
}
//更新当前时间
function getCurrentTime(){
	...
	$('.time-begin').html(totalTime);
	console.log('开始时间'+totalTime);
}
//暂停或播放
var progressTimer;
