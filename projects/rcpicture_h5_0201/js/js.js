...
//			};
			...
			//submit tel
			// $(document).on("tap","#submit .tel-submit",function(event){
			// 	event.preventDefault();
			// 	if ( _this.busy ){
			// 		return false;
			// 	}
			...
			// 	if ( !checkInfo.checkTel(postData.tel) ){
			// 		windowAlert.show("请输入正确的手机号码");
			// 		return false;
			// 	}
			// 	_this.busy = true;
			// 	$.ajax({
			...
			// 			}
			// 			_this.busy = false;
			// 			page.closePower("#edit");
			// 			page.changePower("#submit","#result");
			// 		},
			// 		error:function(){
			// 			windowAlert.show("上传照片时发生错误，请重试");
			// 			_this.busy = false;
			// 		}
			// 	});
			// });
...
//			})
		...
		modelData:[
			...
			{"id":2,"width":456,"height":500,"left":50,"top":270,"round":false,
				"text":{"text":"以蒋欣作品 推动平湖生活品质发展","fontSize":18,"lineHeight":20,"textLeft":50,"textTop":780,"length":23,"color":"#585858","textAlgin":"left","fontWeight":"bold"}
			},
			...
			{"id":8,"width":456,"height":590,"left":50,"top":30,"round":false,
			...
			{"id":12,"width":440,"height":525,"left":116,"top":308,"round":false,
				"text":{"text":"洞悉平湖的抱负与骄傲\n共同见证世界的蜕变","fontSize":18,"lineHeight":40,"textLeft":270,"textTop":190,"length":15,"color":"#003e75","textAlgin":"left","fontWeight":"bold"}
			},
		...
		shengcheng:function(){
			...
			if ( _this.currentModel.text ){
				var text = $("#content").find("textarea")[0].value;
				for ( var i=0;i<_this.errText.length;i++ ){
					if ( text.indexOf(_this.errText[i]) > -1 ){
						windowAlert.show("请不要录入敏感词汇");
						return false;
						break;
					}
				}
		    ...
		    uploadToQiniu(_this.canvas.toDataURL("image/jpeg",1),function(url, shareimgsrc){
				...
				$.ajax({
					type:"get",
					url:"http://cdn.h5case.com.cn/" + url + "?qpolitician",
					dataType:"json",
					data:_this.postData,
					async:false,
					timeout:10000,
					success:function(data){
						if ( data.code != 0 ){
							windowAlert.close("图片在政治鉴定时发生错误");
							_this.busy = false;
							goon = false;
							return false;
						}
						for ( var i=0;i<data.result.detections.length;i++ ){
							if ( data.result.detections[i].value.name ){
								windowAlert.close("照片中发现有政治敏感人物");
								_this.busy = false;
								goon = false;
								return false;
							}
					...
					error:function(xhr,textStatus){
						if( textStatus == "timeout" ){
							windowAlert.close("提交超时，请重试");
						}
						else{
							windowAlert.close("网络错误，请重试");
						}
						goon = false;
						_this.busy = false;
					}
				...
				$.ajax({
					type:"get",
					url:"http://cdn.h5case.com.cn/" + url + "?qpulp",
					dataType:"json",
					data:_this.postData,
					async:false,
					timeout:10000,
					success:function(data){
						if ( data.code != 0 ){
							windowAlert.close("图片在黄图鉴定时发生错误");
							_this.busy = false;
							goon = false;
							return false;
						}
						if ( data.result.label == 0 ){
							windowAlert.close("照片中发现有黄色内容");
							_this.busy = false;
							goon = false;
							return false;
						}
		...
		complete:function(){
			...
			if ( _this.imgarr.bg && _this.imgarr.fixed && _this.dragdown() ){
			    ...
			    uploadToQiniu(canvas.toDataURL("image/jpeg",1),function(url, shareimgsrc){
					...
					$.ajax({
						type:"get",
						url:"http://wx.bocweb.cn/wangjiaqi/rcpicture_h5_0201/submit",
						data:postData,
						dataType:"json",
						async:true,
						timeout:10000,
